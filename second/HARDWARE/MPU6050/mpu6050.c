...
//	MPU_Write_Byte(MPU_FIFO_EN_REG,0X10);	//打开fifo的zgbuff
//	MPU_Write_Byte(MPU_INTBP_CFG_REG,0X80);	//INT引脚低电平有效
//	res=MPU_Read_Byte(MPU_DEVICE_ID_REG); 
//	if(res==MPU_ADDR)//器件ID正确
//	{
//		MPU_Write_Byte(MPU_PWR_MGMT1_REG,0X03);	//设置CLKSEL,PLL Z轴为参考
//		MPU_Write_Byte(MPU_PWR_MGMT2_REG,0X38);	//加速度计standyby模式
...
// 
//    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO|RCC_APB2Periph_GPIOC,ENABLE); //外部中断，需要使能 AFIO 时钟和PC时钟
//	
//	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;				 //LED0-->PC-10 端口配置
//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		     //上拉输入
//	GPIO_Init(GPIOC, &GPIO_InitStructure);					 //根据设定参数初始化PC.10
...
//		while(mpuget_yaw(&set_yaw)!=0);  //mpu6050中断读取
//		while(mpu_get_int_status(tmp));  //清除中断标志位
//    } 
//    EXTI_ClearITPendingBit(EXTI_Line10);    //清除 LINE10 线路挂起位 
//	
//} 


void TIM3_Init(u16 arr,u16 psc)    //中断读取mpu6050的值
{  

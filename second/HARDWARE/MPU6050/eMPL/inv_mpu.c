...
u8 mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 2; 
		res=mpu_set_sample_rate(500);	//设置采样率
		if(res)return 3; 
...
//		    DMP_FEATURE_GYRO_CAL);
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT	              //更改设置dmp功能
		 |DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL
		);
		if(res)return 6; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//设置DMP输出速率(最大不超过200Hz)100hz
		if(res)return 7;   
